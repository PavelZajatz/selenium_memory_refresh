...
class TestCookies:
    ...
    def test_max_expiry_cookie(self):
        """
        Tests the retrieval of the text from a page with the cookie that has the maximum expiry value.

        This test navigates to a specified URL, collects all links on the page, and then visits each link
        to find the cookie with the maximum expiry value. After finding the correct link, it asserts that
        the text of the 'result' element matches the expected value.

        Steps:
        1. Open the main URL specified by `CookiesLocators.URL_2`.
        2. Retrieve all URLs from anchor elements (`<a>`).
    ...
    def test_sum_secret_cookies(self):
        """
        Tests the summation of values from cookies with 'secret_cookie_' in their names.

        This test navigates to the specified URL, retrieves all cookies, filters those
        containing 'secret_cookie_' in their names, and sums their values. Finally, it
        asserts that the total sum matches the expected value.
    ...
    def test_find_youngest_hacker(self, cookies):
        """
        Test method to find the cookie value of the youngest hacker with the most programming languages.
